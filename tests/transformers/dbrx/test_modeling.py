...
from paddlenlp.transformers import DbrxConfig, DbrxForCausalLM, DbrxModel
...
class DbrxModelTester:
    def __init__(
        ...
        self.parent: DbrxModelTest = parent
    ...
    def get_config(self) -> DbrxConfig:
        return DbrxConfig(
    ...
    def create_and_check_model(
        self, config: DbrxConfig, input_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
...
class DbrxModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (DbrxModel, DbrxForCausalLM)
    all_generative_model_classes = {DbrxForCausalLM: (DbrxModel, "Dbrx")}

    def setUp(self):
        super().setUp()

        self.model_tester = DbrxModelTester(self)
        self.config_tester = ConfigTester(self, config_class=DbrxConfig, vocab_size=256, hidden_size=24)
    ...
    def test_dbrx_lm_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
