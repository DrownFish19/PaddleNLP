    ...
    def _load_from_checkpoint(self, resume_from_checkpoint=None):
        ...
        if self.args.unified_checkpoint:
            if resume_from_checkpoint is not None:
                ...
                if use_unified_checkpoint:
                    self.timers("load_unified_checkpoint").start()
                    load_unified_checkpoint(
                        self.args,
                        self.model,
                        self.optimizer,
                    ...
                    self.timers("load_unified_checkpoint").stop()
        ...
        if self.args.should_load_sharding_stage1_model:
            ...
            if resume_from_checkpoint is not None and self.args.dataset_rank == 0:
                self.timers("load_paddle_checkpoint").start()
                weights_file = os.path.join(
                ...
                self.timers("load_paddle_checkpoint").stop()
    ...
    def _save_checkpoint(self, model, metrics=None):
        ...
        if self.args.use_hybrid_parallel:
            if self.dp_group.rank <= 0:
                ...
                if self.args.unified_checkpoint:
                    self.timers("save_unified_optimizer").start()
                    save_unified_optimizer(
                        self.args,
                        self.model,
                        self.optimizer,
                    ...
                    self.timers("save_unified_optimizer").stop()
                else:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None, merge_tensor_parallel=False):
        ...
        # TODO: @ZHUI unifiy unwrap_model(self.model) and self.model
            ...
            if isinstance(self.model, PretrainedModel) and self.args.should_save_sharding_stage1_model:
                ...
                self.timers("save_paddle_checkpoint").start()
                self.model.save_pretrained(
                    output_dir,
                    merge_tensor_parallel=merge_tensor_parallel,
                    variant=self.args.weight_name_suffix,
                ...
                self.timers("save_paddle_checkpoint").stop()
        if self.args.should_save_sharding_stage1_model:
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if self.args.should_load_sharding_stage1_model:
            ...
            if not use_unified_checkpoint:
                self.timers("load_paddle_optimizer").start()
                if self.args.data_parallel_rank == 0:
                    ...
                    opt_state_dict = None
                self.timers("load_paddle_optimizer").stop()
            else:
                self.timers("load_unified_optimizer").start()
                ...
                self.timers("load_unified_optimizer").stop()
